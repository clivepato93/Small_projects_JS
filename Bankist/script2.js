...
const displayMovements = function (acc,sort=0) {
  ...
  const movs = sort===1? acc.movements.slice().sort((a,b)=> a-b):
  sort===2?acc.movements.slice().sort((a,b)=> b-a): acc.movements;
...
const calcDisplaySummary = function (acc) {
  ...
  const outgoings=  Math.abs(acc.movements
  .filter(mov => mov <  0)
  .reduce((acc,curr) => acc+curr,0))
...
btnLogin.addEventListener('click',function (event) {
  ...
  if(currentAccount?.pin===Number(inputLoginPin.value)){
    ...
    // useful link for different time zones 
...
btnLoan.addEventListener('click', function (e) {
  ...
  if (loanRequest>0 && currentAccount.movements.some(mov => mov >= loanRequest*0.1)){
    setTimeout( function delayLoan(){ 
     
    currentAccount.movements.push(loanRequest)
    currentAccount.movementsDates.push(new Date().toISOString());

    updateUI(currentAccount);},4000);
    clearInterval(timer);
    timer =startLogOutTimer();
